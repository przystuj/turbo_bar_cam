-- this is an example script which I used for this replay https://bar-rts.com/replays/bb936469494e23ac3618b99110f57e4d

return {
    {
        frame="5",
        commands="skip 390"
    },
    {
        frame="11700",
        commands="turbobarcam_anchor_adjust_params set;DURATION,25"
    },
    {
        frame="11730",
        commands="turbobarcam_anchor_focus 1"
    },
    {
        frame="11745",
        commands="turbobarcam_anchor_focus reset"
    },
    {
        frame="11760",
        commands="turbobarcam_anchor_focus 2"
    },
    {
        frame="11800",
        commands="turbobarcam_script_show_players_list"
    },
    {
        frame="12010",
        commands="turbobarcam_toggle_unit_follow_camera 215"
    },
    {
        frame="12015",
        commands="turbobarcam_unit_follow_toggle_combat_mode"
    },
    {
        frame="12020",
        commands="turbobarcam_script_show_players_list"
    },
    {
        frame="12120",
        commands="togglelos"
    },
    {
        timestamp="6:50",
        commands="turbobarcam_unit_follow_toggle_combat_mode"
    },
    {
        timestamp="7:13",
        commands="turbobarcam_unit_follow_toggle_combat_mode"
    },
    {
        frame="13487",
        commands="turbobarcam_unit_follow_reset_attack_state 6"
    },
    {
        timestamp="8:08",
        commands="setspeed 5"
    },
    {
        timestamp="8:09",
        commands="turbobarcam_unit_follow_reset_attack_state 7"
    },
    {
        timestamp="8:40",
        commands="setspeed 1"
    },
    {
        timestamp="9:16",
        commands="setspeed 6"
    },
    {
        timestamp="10:05",
        commands="setspeed 1"
    },
    {
        timestamp="14:00",
        commands="setspeed 6"
    },
    {
        timestamp="14:01",
        commands="turbobarcam_unit_follow_reset_attack_state 20"
    },
    {
        timestamp="15:44",
        commands="setspeed 1"
    },
    {
        frame="34053",
        commands="turbobarcam_projectile_camera_cycle forward static"
    },
    {
        frame="34179",
        commands="turbobarcam_toggle_unit_follow_camera 215"
    },
    {
        frame="34185",
        commands="turbobarcam_unit_follow_toggle_combat_mode"
    },
    {
        frame="35515",
        commands="setspeed 3"
    },
    {
        frame="35981",
        commands="setspeed 1"
    },
    {
        frame="37306",
        commands="setspeed 4"
    },
    {
        frame="38260",
        commands="setspeed 1"
    },
    {
        timestamp="21:59",
        commands="turbobarcam_unit_follow_reset_attack_state 1"
    },
    {
        frame="39573",
        commands="turbobarcam_projectile_camera_cycle forward static"
    },
    {
        frame="39768",
        commands="turbobarcam_toggle_unit_follow_camera 215"
    },
    {
        frame="39770",
        commands="turbobarcam_unit_follow_toggle_combat_mode"
    },
    {
        frame="39841",
        commands="turbobarcam_anchor_adjust_params set;DURATION,65"
    },
    {
        frame="39900",
        commands="turbobarcam_anchor_focus 3"
    },
    {
        frame="60000",
        commands="dummy"
    },
}
