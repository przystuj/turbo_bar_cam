...
function DollyCamEditor.addWaypointAtCurrentPosition()
    STATE.dollyCam.route = STATE.dollyCam.route or {points = {}}

    -- Get current camera position and rotation
    local camState = CameraManager.getCameraState("DollyCamEditor.addWaypointAtCurrentPosition")
    local position = {
        x = camState.px,
        y = camState.py,
        z = camState.pz
    ...
    if STATE.dollyCam.isNavigating then
        ...
        local currentDistance = STATE.dollyCam.currentDistance
        local traversedDistance = 0
        
        for i, segmentDist in ipairs(STATE.dollyCam.route.segmentDistances) do
            if traversedDistance + segmentDist >= currentDistance then
                insertIndex = i + 1
                break
            end
...
function DollyCamEditor.addOrEditWaypointAtCurrentPosition()
    ...
    if closestIndex then
        ...
        -- Add new waypoint
        local success, index = DollyCamEditor.addWaypointAtCurrentPosition()
