...
| **Focus Anchor**<br>`turbobarcam_anchor_focus` | Smoothly transitions the camera to a previously saved anchor point. Move to next/previous anchor by using `next`/`prev` parameter | `Shift+F1`<br/>`Shift+F2`<br/>`Shift+F3`<br/>`Shift+F4` | `prev`<br/>`next`<br/>`reset`<br/>`1` |
